...
class MemoryPressureTracker {
  ...
  */
  ...
  // while loop, the body of the while could use quite some back memory than the
 ...
 public:
  ...
  struct SchedulingState : public SchedulerCore::SchedulingState {
    ...
    SchedulingState(
        ...
        const MemoryPressureMetadata* pressure_metadata,
        const SchedulerConfig& config)
        : sched_graph(std::make_unique<HloScheduleGraph>(
              &instr_sequence->instructions(), scheduling_context)),
  ...
  absl::Status ResetScheduler(const HloModule* module) override {
    module_pressure_state_->ResetPressureStates(&pressure_metadata_);
    return absl::OkStatus();
  }
