...
void ModulePressureState::InitializePressureStates(
  ...
  absl::flat_hash_map<const HloComputation*,
                      std::unique_ptr<MemoryPressureMetadata>>
      temp_metadata;
  if (pressure_metadata == nullptr) {
    pressure_metadata = &temp_metadata;
  } else {
    pressure_metadata->clear();
  }
  ResetPressureStates(pressure_metadata);
}
...
void ModulePressureState::ResetPressureStates(
    absl::flat_hash_map<const HloComputation*,
                        std::unique_ptr<MemoryPressureMetadata>>*
        pressure_metadata) {
  CHECK(pressure_metadata != nullptr);
  memory_pressure_states_.clear();
  std::function<void(HloComputation*, const LiveBufferSet&)>
      process_computation = [this, &process_computation, pressure_metadata](
        ...
        auto it = pressure_metadata->find(computation);
        if (it == pressure_metadata->end()) {
          ...
          it = pressure_metadata->emplace(computation, std::move(new_metadata))
                   .first;
        }
...
absl::Status DefaultSchedulerCore::InitializeScheduler(
  ...
  module_pressure_state_->InitializePressureStates(&pressure_metadata_);
...
DefaultSchedulerCore::MakeSchedulingState(const HloComputation* computation) {
  ...
  CHECK(it != pressure_metadata_.end())
